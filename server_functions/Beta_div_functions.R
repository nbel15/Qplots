...
# Script: Beta diversity (server functions)
...
source("analysis_functions/suppl_functions.R")
...
observeEvent(input$beta.show.dist.key, {
  showModal(modalDialog(
    title = paste("Dissimilarity matrix : ", input$dist.method, sep = ""),
    div(style="display: inline-block;vertical-align:top; width: 100px;",downloadButton("beta.dist.matrix", "Download")),
...
beta.div.plot.fun <- function(){
  ...
  P <- ggscatter(beta.ordi.list[["ordi.tab"]], x = "axis1", y = "axis2",# facet.by = "Sex",
            ...
            #title = "title", 
            caption = paste("NMDS plot of Microbial Profiles","\n","(p-value: ", beta.ordi.list$adonis,")",sep="")
  )
  return(P)
  } else {
...
beta.div.pair.plot.fun <- function(){
  ...
  if(!is.null(beta.pair.plot)){
    ...
    P <- ggplot(data = beta.pair.plot[["df.select"]], aes(Groups1, Groups2, fill = p_range))+
      ...
      #scale_fill_manual(values = wes_palette(name = "FantasticFox1",n = 3, type = "discrete"))+
      #scale_fill_manual(values = wes_palette(name = "Moonrise2",n = 3, type = "discrete"))+
      theme_minimal()+ 
      geom_text(aes(label = Pval), color = "black", size = 3)+
      coord_fixed()+
